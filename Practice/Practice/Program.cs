...
class Result
{
    ...
    public int EqullibriumIndex(List<int> inputs)
    {
        var totalSumEven = 0;
        var totalSumOdd = 0;
        ...
        {
            if (i % 2 == 0)
                totalSumEven += inputs[i];
            else
                totalSumOdd += inputs[i];
        }
        ...
        {
            currentEven = preFixSumEven + totalSumOdd - preFixSumOdd - (i % 2 != 0 ? inputs[i] : 0);
            currentOdd = preFixSumOdd + totalSumEven - preFixSumEven - (i % 2 != 0 ? 0 : inputs[i]);
            ...
            if (i % 2 == 0)
            {
                preFixSumEven += inputs[i];
            }
            else
            {
                preFixSumOdd += inputs[i];
            }
        }
        return -1;
    }
    ...
    public int LongestPallindrome(char[] inputString)
    {
        ...
        {
            CheckInput(i, i, ref max, inputString);

            CheckInput(i, i + 1, ref max, inputString);
        }
        return max;
    }


    public void CheckInput(int start, int end, ref int max, char[] inputs)
    {
        while (start >= 0 && end < inputs.Length && inputs[start] == inputs[end])
        {
            start--;
            end++;
        }
        var currentLength = end - start - 1;// Even if 1st didn't match it will give correct 
